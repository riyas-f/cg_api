...
func SetAccountRoute(r *mux.Router, db *sql.DB, config *config.Config) {
	...
	linkSteamId := httpx.CreateHTTPHandler(db, config, linkSteamAccountHandler, nil)
	...
	getSteamId := httpx.CreateHTTPHandler(db, config, getUserSteamIDHandler, &cors.Options{
		AllowedOrigins:   []string{"*"},
		AllowedMethods:   []string{"GET", "POST", "PUT", "DELETE"},
		AllowedHeaders:   []string{"Content-Type", "Authorization"},
		ExposedHeaders:   []string{"Content-Length"},
	...
	rollbackSteamID := httpx.CreateHTTPHandler(db, config, rollbackSteamLinkHandler, nil)
