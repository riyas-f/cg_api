...
# get root ca cert from google bucket 
curl ${GCP_CERT_FILE_BUCKET_URL} -o $CERT_FILE_PATH
gcloud secrets versions access latest --secret=${GCP_PRIVATE_KEY_SECRET_NAME} > $PRIVATE_KEY_PATH
gcloud secrets versions access latest --secret=${GCP_PRIVATE_KEY_PASSPHRASE_SECRET_NAM} > /tmp/passphrase
